...
static void update_bitmap_score(struct queue_entry* q) {
  ...
  u32 i;
  u64 fav_factor = q->exec_us * (u32)(q->len * q->grad * 10); //* fValue * 1000; //Multiply some gradient thing here
...
static void maybe_update_plot_file(double bitmap_cvg, double eps) {
  ...
  static u64 prev_qc, prev_uc, prev_uh, prev_execs;

  if (prev_qp == queued_paths && prev_pf == pending_favored && 
      prev_pnf == pending_not_fuzzed && prev_ce == current_entry &&
      prev_qc == queue_cycle && prev_uc == unique_crashes &&
      prev_uh == unique_hangs && prev_md == max_depth && 
      prev_execs == total_execs) return;
...
int main(int argc, char** argv) {
  ...
  while (1) {
    ...
    memset(buffer, '\0', sizeof(buffer));
    recv(client_fd, buffer, 1024, 0);
    printf("waiting fuzz_one\n");
    double input = atof(buffer);

    queue_cur->grad = input;
